...
void initializeBodies(struct body *bods)
{
    ...
    // STARS
    velocity = 0.67 * sqrt((G * SOLAR_MASS) / (4 * BINARY_SEPARATION * AU_TO_METRES));
    // STAR 1
    current = &bods[0];
    ...
    */
    ...
    for (int index = 1; index < NUM_BODIES; index++)
    {
        ...
        velocity = pow(((G * (SOLAR_MASS + ((radius - INNER_BOUND) / SYSTEM_SIZE) * EXTRA_MASS * SOLAR_MASS)) / (radius * AU_TO_METRES)), 0.5);
        current = &bods[index];
...
void singleInteraction(struct body *a, struct body *b)
{
    ...
    double distance = magnitude(positive_difference);
...
#pragma omp for
    for (int bIndex = 0; bIndex < NUM_BODIES; bIndex++)
    {
        ...
        current->accel.y = 0.0;
        current->accel.z = 0.0;
        current->position.x += TIME_STEP * current->velocity.x / AU_TO_METRES;
        current->position.y += TIME_STEP * current->velocity.y / AU_TO_METRES;
        current->position.z += TIME_STEP * current->velocity.z / AU_TO_METRES;
    }
    if (DEBUG_INFO)
    {
...
void colorDot(double x, double y, double vMag, double *hdImage)
{
    ...
    double velocityMin = sqrt(0.8 * (G * (SOLAR_MASS + EXTRA_MASS * SOLAR_MASS)) / (SYSTEM_SIZE * AU_TO_METRES));
    // MIN_VEL_COLOR;
    if (vMag < velocityMin)
        return;
        ...
        for (int j = -DOT_SIZE / 2; j < DOT_SIZE / 2; j++)
        {
            ...
            double cFactor = PARTICLE_BRIGHTNESS /
                             (pow(exp(pow(PARTICLE_SHARPNESS *
                                              (xP + i - toPixelSpace(x, WIDTH)),
                                          2.0)) +
                                      exp(pow(PARTICLE_SHARPNESS *
                                                  (yP + j - toPixelSpace(y, HEIGHT)),
                                              2.0)),
                                  /*1.25*/ 0.75) +
                              1.0);
